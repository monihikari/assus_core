...
class TimeUtils(object):
    ...
    def localize_datetime(cls, date_time: datetime, time_zone: Optional[str] = None):
        ...
        local_time = (
            pytz.utc.localize(date_time)
            if not date_time.tzinfo else date_time
        )
        return local_time.astimezone(local_tz)

    @classmethod
    def localize_and_format(cls, date_time: datetime, time_zone: Optional[str] = None) -> str:
        return cls.datetime_to_string(
            cls.localize_datetime(date_time, time_zone)
        )
    ...
    def extract_humanized_date(
        ...
        return (
            cls.localize_datetime(date_time, time_zone).strftime('%d %b %Y')
        )
    ...
    @classmethod
    def format_simple_time(cls, input_time: datetime.time):
    ...
    def date_time_plus_days(
        ...
        date_time = (
            TimeUtils.localize_datetime(date_time, time_zone)
            if time_zone else date_time
        )
    ...
    def localize_and_split(cls, date_time: datetime, time_zone: str):
        local_date_time = cls.replace_timezone(
            date_time=date_time,
